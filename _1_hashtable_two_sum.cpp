...
class Solution0001 {
 public:
  ...
  std::vector<int> twoSum(std::vector<int> &nums, int target) {
    // Currently, clang does not support polymorphic memory resource library
    // which is in C++17 std::array<std::byte, 2048> stack_buffer;
    // std::pmr::monotonic_buffer_resource pool { std::data(stack_buffer),
    // std::size(stack_buffer) }; std::pmr::unordered_map<int, int> hash_map {
    // &pool };
